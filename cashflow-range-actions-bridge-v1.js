(() => {
  ...
  const statusLabels = { planned:'מתוכנן', forecast:'צפוי', completed:'בוצע', actual:'בוצע', paid:'שולם', settled:'נסלק', deferred:'נדחה', overdue:'באיחור', cancelled:'בוטל', unconfirmed:'לא מאומת' };
  const scopeLabels = { business:'עסקית', personal:'אישית' };
  let items = [];
      ...
      <div id="range-bridge-modal" class="range-bridge-modal" hidden>
        <section class="range-bridge-dialog">
          ...
          <form id="range-bridge-form">
            ...
            <label>שיוך
              <span class="range-scope-options">
                <label class="range-scope-option"><input type="radio" name="range-financial-scope" value="business" checked><span>עסקית</span></label>
                <label class="range-scope-option"><input type="radio" name="range-financial-scope" value="personal"><span>אישית</span></label>
              </span>
            </label>
  ...
  function setScope(scope) {
    const input = document.querySelector(`input[name="range-financial-scope"][value="${scope === 'personal' ? 'personal' : 'business'}"]`);
    if (input) input.checked = true;
  }
  function getScope() {
    return document.querySelector('input[name="range-financial-scope"]:checked')?.value || 'business';
  }
  ...
  function openEdit(id) {
    ...
    setScope(item.financial_scope || 'business');
  ...
  async function updateItem(item, nextStatus) {
    const { error } = await client.rpc('update_cashflow_entry_full_v2', {
      p_entry_id:item.id, p_date:item.date, p_type:item.type, p_amount:Number(item.amount),
      p_description:item.description || 'תנועה', p_status:nextStatus,
      p_note:item.action_note || null, p_payment_method:item.payment_method || 'unknown',
  ...
  async function quickConfirm(id, button) {
    ...
    try { await updateItem(item, 'completed'); await loadAndRender(); }
    catch (error) { console.error(error); button.disabled=false; button.textContent=original; alert(`האישור נכשל: ${error.message}`); }
  }
  ...
  async function save(event) {
    ...
    try {
      ...
      if (id) {
        const item = items.find(row => row.id === id);
        const { error } = await client.rpc('update_cashflow_entry_full_v2', {
          p_entry_id:id, ...payload,
          p_status:document.getElementById('range-bridge-status-select').value,
          p_payment_method:item?.payment_method || 'unknown'
        });
        if (error) throw error;
      } else {
        const { error } = await client.rpc('create_cashflow_entry_v3', { ...payload, p_payment_method:'unknown' });
        if (error) throw error;
      }
  ...
  function render() {
    document.querySelectorAll('[data-day-card][data-cashflow-date]').forEach(card => {
      ...
      ['income','expense'].forEach((type,index) => {
        ...
        if (!list) return;
        const rows = items.filter(item => item.date === date && item.type === type && (item.financial_scope || 'business') === 'business');
