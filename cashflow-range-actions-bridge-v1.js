  ...
  function render() {
    document.querySelectorAll('[data-day-card][data-cashflow-date]').forEach(card => {
      ...
      ['income', 'expense'].forEach((type, index) => {
        ...
        list.innerHTML = rows.length ? rows.map(item => `<li class="range-entry-row"><div><strong>${esc(item.description || 'תנועה')}</strong><small>${money(item.amount)} · ${esc(item.status || 'planned')}</small></div><div class="range-entry-actions"><button type="button" class="range-entry-btn" data-range-edit="${esc(item.id)}">עריכה</button></div></li>`).join('') : '<li class="empty">אין תנועות</li>';
      });
