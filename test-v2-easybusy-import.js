(() => {
  ...
  let payload=null,previewData=null;
  ...
  const methodLabel=m=>({credit_card:'אשראי',cash:'מזומן',check:'צ׳ק',bit:'Bit',paybox:'PayBox',bank_transfer:'העברה בנקאית',other:'אחר'})[m]||m||'לא ידוע';

  function ensureInsights(){
    if($('import-insights'))return $('import-insights');
    const box=document.createElement('section');box.id='import-insights';box.style.margin='14px 0';
  ...
  function parsePayments(text,doc){
    ...
    return String(text).split(',').map((part,i)=>{
      ...
      const label=m[1].trim(),amount=Number(m[2].replace(/,/g,''));let method='other';
  ...
  async function parseZip(file){
    ...
    const inc=findBook(books,'Incomes_')||findBook(books,'Incomes'),intake=findBook(books,'Intakes_')||findBook(books,'Intakes'),zbook=findBook(books,'ZIndexes_')||findBook(books,'ZIndexes');
    ...
    const intakeMap=new Map(intakes.map(r=>[String(r.IntakeNumber??'').trim(),r])),zByDate=new Map(zs.map(r=>[dateOnly(r.Date),r]));
    const sales=incomes.map(r=>{const doc=String(r.IncomeNumber??'').trim(),detail=intakeMap.get(doc)||{},d=dateOnly(r.Date),z=zByDate.get(d)||{};return {transaction_datetime:localIso(r.Date),transaction_date:d,z_index:z.ZIndexNumber||null,invoice_number:doc,notes:r.Remarks||detail.Remarks||null,employee_name:null,customer_name:r.CustomerName||detail.CustomerName||null,payment_method:r.PaymentDescription||detail.PaymentDescription||null,amount:Number(r.TotalAmount||0),debt:0,easybusy_customer_id:r.CustomerId?String(r.CustomerId):null,identity_number:r.IdentifyNumber?String(r.IdentifyNumber):null,products_services:detail.Details||null,prepayment:0,source_reference:`easybusy_daily_zip:${doc}`,raw_data:{income_type:r.IncomeType,vat:Number(r.Vat||0),allocation_number:r.AllocationNumber||null,import_source:'daily_zip'}}}).filter(r=>r.invoice_number&&r.transaction_date);
    ...
    if(!sales.length&&!days.length)throw new Error('לא נמצאו רשומות תקינות בדוחות');return {sales,payments,days};
  }
  function reconciliation(){
    const byDate=new Map();for(const s of payload.sales){const x=byDate.get(s.transaction_date)||0;byDate.set(s.transaction_date,x+Number(s.amount||0))}
    return payload.days.map(d=>{const docs=byDate.get(d.work_date)||0,z=Number(d.sales||0),diff=docs-z;return {...d,documents_total:docs,difference:diff,ok:Math.abs(diff)<0.01}})
  }
  function renderInsights(){
    const box=ensureInsights(),rec=reconciliation(),methods=previewData?.payment_methods||[];
    box.innerHTML=`<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:10px;margin-bottom:12px">
      <article style="border:1px solid #eadff0;border-radius:14px;padding:13px"><span>מסמכים חדשים</span><strong style="display:block;color:#18794e;font-size:1.3rem">${Number(previewData?.new_sales||0)}</strong></article>
      <article style="border:1px solid #eadff0;border-radius:14px;padding:13px"><span>מסמכים קיימים</span><strong style="display:block;color:#69328e;font-size:1.3rem">${Number(previewData?.existing_sales||0)}</strong><small>${Number(previewData?.changed_sales||0)} ישתנו</small></article>
      <article style="border:1px solid #eadff0;border-radius:14px;padding:13px"><span>רכיבי תשלום חדשים</span><strong style="display:block;color:#18794e;font-size:1.3rem">${Number(previewData?.new_payments||0)}</strong></article>
      <article style="border:1px solid #eadff0;border-radius:14px;padding:13px"><span>רכיבי תשלום קיימים</span><strong style="display:block;color:#69328e;font-size:1.3rem">${Number(previewData?.existing_payments||0)}</strong></article>
    </div>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:10px">
      <article style="border:1px solid #eadff0;border-radius:14px;padding:13px"><b>פירוט אמצעי תשלום</b>${methods.length?methods.map(x=>`<div style="display:flex;justify-content:space-between;margin-top:7px"><span>${esc(methodLabel(x.method))} · ${Number(x.count||0)}</span><strong>${money(x.amount)}</strong></div>`).join(''):'<p>לא זוהו רכיבי תשלום.</p>'}</article>
      <article style="border:1px solid #eadff0;border-radius:14px;padding:13px"><b>התאמה לסיכומי Z</b>${rec.length?rec.map(x=>`<div style="margin-top:8px;padding:8px;border-radius:10px;background:${x.ok?'#edf9f2':'#fff4e5'}"><strong>${esc(new Date(x.work_date+'T12:00:00').toLocaleDateString('he-IL'))}</strong><div>מסמכים ${money(x.documents_total)} · Z ${money(x.sales)}</div><div>${x.ok?'✓ תואם':'פער '+money(x.difference)}</div></div>`).join(''):'<p>לא נמצאו ימי Z.</p>'}</article>
    </div>`;
  }
  function render(){
    ...
    renderInsights();$('preview-wrap').hidden=false;$('preview-body').innerHTML=payload.sales.map(r=>`<tr><td>${esc(new Date(r.transaction_date+'T12:00:00').toLocaleDateString('he-IL'))}</td><td>${esc(r.invoice_number)}</td><td>${esc(r.customer_name)}</td><td>${money(r.amount)}</td><td>${esc(r.payment_method)}</td><td>${esc(r.products_services||'—')}</td></tr>`).join('');$('import-button').disabled=false;
  }
  $('zip-file').addEventListener('change',async e=>{
    const file=e.target.files?.[0];if(!file)return;$('file-name').textContent=file.name;payload=null;previewData=null;$('import-button').disabled=true;$('summary').hidden=true;$('preview-wrap').hidden=true;if($('import-insights'))$('import-insights').innerHTML='';statusBox.className='status';statusBox.textContent='קורא, מנתח ובודק כפילויות מול Supabase…';
    try{payload=await parseZip(file);const{data,error}=await client.rpc('preview_easybusy_daily_zip_v1',{p_payload:payload});if(error)throw error;previewData=data||{};render();const bad=reconciliation().filter(x=>!x.ok).length;statusBox.className=bad?'status':'status ok';statusBox.textContent=bad?`הקובץ נותח, אך נמצאו ${bad} פערים מול Z. יש לבדוק לפני ייבוא.`:'הקובץ נותח ונבדק מול המסד. יש לבדוק את התצוגה ולאשר ייבוא.'}catch(err){statusBox.className='status error';statusBox.textContent=err.message||String(err)}
  });
