(() => {
  ...
  function periodChanged(){const detail=getPeriod();window.dispatchEvent(new CustomEvent('beautix:period-change',{detail}));if(state.client&&state.tab==='overview')loadOverview()}
  ...
  async function loadOverview(){if(!state.client)return;const period=getPeriod(),panel=$('panel-overview');panel.setAttribute('aria-busy','true');const{data,error}=await state.client.rpc('get_overview_period_v1',{p_start:period.start,p_end:period.end});panel.removeAttribute('aria-busy');if(error){panel.querySelector('.summary-grid').innerHTML=`<article class="summary-card"><span>שגיאת טעינה</span><strong>לא ניתן לטעון</strong><p>${esc(error.message)}</p></article>`;return}renderOverview(data||{})}
  ...
  async function loadCashflow(){if(!state.client)return;const range=currentCashflowRange();if(!range.start||!range.end)return;const panel=$('panel-cashflow');panel.setAttribute('aria-busy','true');const{data,error}=await state.client.rpc('get_cashflow_range_v1',{p_start:range.start,p_end:range.end});panel.removeAttribute('aria-busy');if(error){$('cashflow-summary').innerHTML=`<article class="summary-card"><span>שגיאת טעינה</span><strong>לא ניתן לטעון</strong><p>${esc(error.message)}</p></article>`;return}renderCashflow(data||{})}
  ...
  async function boot(){wire();renderPeriod();const initial=resolveCashflowRange(state.cashflowRange);$('cashflow-start').value=initial.start;$('cashflow-end').value=initial.end;setTab(location.hash.slice(1)||cfg.defaultTab);await initAuth();window.BEAUTIX={getPeriod,setTab,refreshOverview:loadOverview,refreshCashflow:loadCashflow}}
