(() => {
  ...
  function closeProfileMenu(){ profileMenu.hidden=true; profileMenuButton.setAttribute("aria-expanded","false"); }
  function openProfileMenu(){ profileMenu.hidden=false; profileMenuButton.setAttribute("aria-expanded","true"); }
  function toggleProfileMenu(){ profileMenu.hidden?openProfileMenu():closeProfileMenu(); }
  function closeProfileModal(){ profileModal.hidden=true; document.body.classList.remove("modal-open"); profileStatus.hidden=true; profileAvatarFile.value=""; }
  ...
  async function loadProfile(session){
    ...
    try{
      ...
      return profile;
    }catch(error){ console.warn("Unable to load user profile",error); return profile; }
  }
  ...
  async function updateActiveUser(session){
    ...
    if(profile.avatar_path){
      ...
      }catch(error){ console.warn("Unable to load avatar",error); }
    }
    return profile;
  }
  ...
  async function openProfileEditor(){
    ...
    profileDisplayName.value=profile.display_name?.trim()||data.session.user.email||"";
    profileStatus.hidden=true;
    profileModal.hidden=false;
    document.body.classList.add("modal-open");
    setTimeout(()=>profileDisplayName.focus(),0);
  }
  ...
  function renderSalesInsights(report){ const sales=Number(report.sales?.income||0), target=Number(report.settings?.monthly_sales_target||report.sales?.target||0); const latest=report.sales?.latest?new Date(`${String(report.sales.latest).slice(0,10)}T00:00:00`):new Date(); const day=Math.max(1,latest.getDate()), monthEnd=new Date(latest.getFullYear(),latest.getMonth()+1,0).getDate(); const daily=sales/day, forecast=daily*monthEnd, progress=target>0?sales/target*100:0, gap=target-sales; setText("sales-note",`${numberText(progress,1)}% מהיעד הפעיל`); setText("sales-progress",`${numberText(progress,1)}%`,progress>=100?"pos":progress>=80?"warn":null); setProgress("sales-progress-bar",progress,progress>=100?"green":progress>=80?"amber":"pink"); setText("sales-progress-note",progress>=100?`היעד הושג ונחצה ב־${money(Math.abs(gap))}`:`נותרו ${money(Math.max(gap,0))} ליעד`); setText("sales-daily-rate",money(daily)); setText("sales-daily-rate-note",`ממוצע לפי ${day} ימים בחודש`); setText("sales-forecast",money(forecast),forecast>=target?"pos":"warn"); setText("sales-forecast-note",forecast>=target?`צפי לסיום מעל היעד ב־${money(forecast-target)}`:`צפי לחוסר של ${money(target-forecast)}`); setText("sales-gap",money(Math.abs(gap)),gap<=0?"pos":"neg"); setText("sales-gap-note",gap<=0?`מעל היעד ב־${money(Math.abs(gap))}`:`נדרש קצב של ${money(gap/Math.max(monthEnd-day,1))} ליום עד סוף החודש`); const milestones=[{amount:50000,date:"2026-07-21"},{amount:75000,date:"2026-07-23"},{amount:target,date:null}]; const list=document.getElementById("sales-milestones"); if(list) list.innerHTML=milestones.map((m)=>`<li class="${m.date?"achieved":"active"}"><span>${money(m.amount)}</span><small>${m.date?`נשבר ב־${dateText(m.date)}`:"יעד פעיל"}</small></li>`).join(""); }
  function renderBusinessKpis(report,checkingBalance){ const settings=report.settings||{}, monthly=report.monthly||{}, sales=Number(report.sales?.income||monthly.sales||0), immediate=Number(monthly.immediate_receipts||monthly.bank_receipts||0), immediateTarget=Number(settings.monthly_revenue_target||57500), dailyTarget=Number(settings.daily_immediate_receipts_target||2500), expenseTarget=Number(settings.monthly_expense_target||20000), checkingTarget=Number(settings.available_cash_target||10000), expenses=Number(monthly.business_expenses||0); const outputVatRaw=report.tax?.output_vat_exact; const incomeTaxRaw=report.tax?.income_tax_estimate; const hasTaxData=outputVatRaw!==null&&outputVatRaw!==undefined&&incomeTaxRaw!==null&&incomeTaxRaw!==undefined; const outputVat=Number(outputVatRaw||0), incomeTax=Number(incomeTaxRaw||0), grossProfit=sales-outputVat-incomeTax, grossProfitMargin=sales>0?grossProfit/sales:0; const latest=report.sales?.latest?new Date(`${String(report.sales.latest).slice(0,10)}T00:00:00`):new Date(); let elapsed=0; for(let d=1;d<=latest.getDate();d+=1){const dt=new Date(latest.getFullYear(),latest.getMonth(),d);if(dt.getDay()!==5&&dt.getDay()!==6)elapsed+=1;} const remaining=Math.max(23-elapsed,0), immediateProgress=immediateTarget>0?immediate/immediateTarget*100:0, gap=Math.max(immediateTarget-immediate,0), required=remaining>0?gap/remaining:gap; const checkingGap=checkingBalance-checkingTarget, expenseProgress=expenses>0&&expenseTarget>0?expenses/expenseTarget*100:0, checkingProgress=checkingTarget>0?Math.max(0,checkingBalance/checkingTarget)*100:0; setText("immediate-receipts",money(immediate),immediate>=immediateTarget?"pos":null); setText("immediate-receipts-progress-note",gap<=0?"היעד הושג":`${numberText(immediateProgress,1)}% מהיעד · חסרים ${money(gap)}`); setProgress("immediate-progress-bar",immediateProgress,immediateProgress>=100?"green":immediateProgress>=75?"amber":"pink"); setText("immediate-receipts-daily-needed",money(required),required<=dailyTarget?"pos":"warn"); setText("immediate-receipts-daily-needed-note",`${remaining} ימי עבודה נותרו · יעד יומי רגיל ${money(dailyTarget)}`); setText("monthly-expenses",expenses>0?money(expenses):"אין נתון",expenses>expenseTarget?"neg":expenses>0?"pos":null); setText("monthly-expense-target",money(expenseTarget)); setText("monthly-expenses-note",expenses>0?(expenses>expenseTarget?`חריגה של ${money(expenses-expenseTarget)}`:`נותרה מסגרת של ${money(expenseTarget-expenses)}`):"ה־RPC עדיין אינו מחזיר הוצאות חודשיות בפועל"); setProgress("expense-progress-bar",expenseProgress,expenses>expenseTarget?"red":"green"); setText("checking-target",money(checkingTarget)); setText("checking-target-note",checkingGap>=0?`מעל היעד ב־${money(checkingGap)}`:`חסרים ${money(Math.abs(checkingGap))} ליעד`); setProgress("checking-target-progress-bar",checkingProgress,checkingBalance>=checkingTarget?"green":checkingBalance>=0?"amber":"red"); setText("checking-gap-note",checkingGap>=0?`העו״ש מעל היעד ב־${money(checkingGap)}`:`חסרים ${money(Math.abs(checkingGap))} ליעד עו״ש של ${money(checkingTarget)}`); setProgress("checking-progress-bar",checkingProgress,checkingBalance>=checkingTarget?"green":checkingBalance>=0?"amber":"red"); if(hasTaxData){ setText("gross-profit",money(grossProfit),grossProfit>=0?"pos":"neg"); setText("gross-profit-margin",`${numberText(grossProfitMargin*100,1)}%`,grossProfitMargin>=0?"pos":"neg"); setText("gross-profit-note",`מכירות ${money(sales)} פחות מע״מ עסקאות ${money(outputVat)} ופחות מס הכנסה ${money(incomeTax)}`); } else { setText("gross-profit","אין נתון","warn"); setText("gross-profit-margin","—"); setText("gross-profit-note","החישוב ממתין לנתוני מע״מ עסקאות ומס הכנסה מלאים ועדכניים"); } }
  ...
  function renderAssets(report){ const positiveAccounts=(report.accounts||[]).filter((x)=>x.scope==="business"&&Number(x.balance||0)>0); const loanItems=Array.isArray(report.loan_items)?report.loan_items:Array.isArray(report.loans?.items)?report.loans.items:Array.isArray(report.loans?.details)?report.loans.details:[]; const loans=report.loans||{}; const cards=[]; const totalDebt=Number(loans.balance||loanItems.reduce((sum,item)=>sum+Number(item.balance||item.current_balance||0),0)); if(totalDebt>0){ cards.push(`<article class="asset-card liability-card debt-total-card"><span>סך החוב הכולל</span><strong class="neg">${money(totalDebt)}</strong><dl><div><dt>מספר הלוואות</dt><dd>${loanItems.length||loans.count||0}</dd></div><div><dt>החזר חודשי כולל</dt><dd>${money(loans.monthly||loanItems.reduce((sum,item)=>sum+Number(item.monthly_payment||item.monthly||0),0))}</dd></div><div><dt>התשלום הקרוב</dt><dd>${dateText(loans.next_payment)}</dd></div></dl></article>`); } if(loanItems.length){ loanItems.forEach((loan,index)=>cards.push(`<article class="asset-card liability-card"><span>${escapeHtml(loan.name||loan.lender||`הלוואה ${index+1}`)}</span><strong class="neg">${money(loan.balance||loan.current_balance||0)}</strong><button class="asset-toggle" type="button" aria-expanded="false">הצג פירוט</button><div class="asset-details" hidden><dl><div><dt>יתרה נוכחית</dt><dd>${money(loan.balance||loan.current_balance||0)}</dd></div><div><dt>יתרה מקורית</dt><dd>${loan.original_balance?money(loan.original_balance):"לא זמין"}</dd></div><div><dt>החזר חודשי</dt><dd>${money(loan.monthly_payment||loan.monthly||0)}</dd></div><div><dt>ריבית</dt><dd>${loan.interest_rate?`${numberText(loan.interest_rate,2)}%`:"לא זמין"}</dd></div><div><dt>תשלום הבא</dt><dd>${dateText(loan.next_payment||loan.next_payment_date)}</dd></div></dl></div></article>`)); } positiveAccounts.forEach((account)=>cards.push(`<article class="asset-card"><span>${escapeHtml(account.name||"נכס עסקי")}</span><strong class="pos">${money(account.balance||0)}</strong><button class="asset-toggle" type="button" aria-expanded="false">הצג פירוט</button><div class="asset-details" hidden><dl><div><dt>סוג</dt><dd>${escapeHtml(account.type||"חשבון")}</dd></div><div><dt>תחום</dt><dd>${escapeHtml(account.scope||"עסקי")}</dd></div><div><dt>מקור</dt><dd>${escapeHtml(account.source||"Supabase")}</dd></div></dl></div></article>`)); if(!cards.length) cards.push('<article class="asset-card data-missing-card"><span>הלוואות ונכסים</span><strong>אין נתון</strong><p>ה־RPC עדיין אינו מחזיר פירוט הלוואות או חשבונות חיוביים.</p></article>'); assetContainer.innerHTML=cards.join(""); assetContainer.querySelectorAll(".asset-toggle").forEach((button)=>button.addEventListener("click",()=>{const panel=button.nextElementSibling;const open=button.getAttribute("aria-expanded")!=="true";button.setAttribute("aria-expanded",String(open));button.textContent=open?"הסתר פירוט":"הצג פירוט";panel.hidden=!open;})); }
  ...
  function renderReport(report){ const accounts=report.accounts||[],checkingAccount=accounts.find((x)=>x.name==="עו״ש עסק – הבינלאומי"),deposit=accounts.find((x)=>x.name==="פיקדון בבינלאומי"),fx=accounts.find((x)=>x.name==="יתרות מט״ח"),checking=Number(checkingAccount?.balance||0),future=(report.future_clearing||[]).filter((x)=>x.status==="expected").reduce((s,x)=>s+Number(x.net||0),0); setMetric("sales",report.sales?.income||0); setMetric("checking",checking,"balance"); setMetric("deposit-balance",deposit?.balance||0); setMetric("fx-balance",fx?.balance||0); setMetric("future-clearing",future); setMetric("output-vat",report.tax?.output_vat_exact||0); setMetric("input-vat",report.tax?.input_vat_estimate||0); setMetric("vat-payable",report.tax?.vat_payable_estimate||0); setMetric("income-tax",report.tax?.income_tax_estimate||0); setMetric("all-tax-reserve",Number(report.tax?.vat_payable_estimate||0)+Number(report.tax?.income_tax_estimate||0)); const days=buildForecast(report); renderSalesInsights(report); renderBusinessKpis(report,checking); renderOpportunities(report); renderAssets(report); const cashflow=renderCashflowInsights(days,checking); renderManagementOverview(report,cashflow); renderDays(days); lastUpdated.textContent=`עודכן מהמסד: ${new Intl.DateTimeFormat("he-IL",{dateStyle:"short",timeStyle:"medium"}).format(new Date())}`; }
  ...
  function showLogin(){ stopRefresh(); closeProfileMenu(); closeProfileModal(); appView.hidden=true; loginView.hidden=false; reportView.hidden=false; loginSubmit.disabled=false; loginSubmit.textContent="כניסה"; activeUser.textContent="—"; setAuthPanel("login"); }
  ...
  loginForm.addEventListener("submit",async(event)=>{ event.preventDefault(); loginError.hidden=true; loginSubmit.disabled=true; loginSubmit.textContent="מתחבר..."; try{ const email=document.getElementById("email").value.trim(); const password=passwordInput.value; const remember=rememberDevice.checked; localStorage.setItem("beautix-remember-device",String(remember)); const {data,error}=await client.auth.signInWithPassword({email,password}); if(error) throw error; if(!data.session) throw new Error("לא נוצרה התחברות פעילה"); await showReport(data.session); }catch(error){ console.error(error); loginError.textContent=friendlyAuthError(error); loginError.hidden=false; loginSubmit.disabled=false; loginSubmit.textContent="כניסה"; } });
  forgotPasswordAction.addEventListener("click",()=>{ forgotEmail.value=document.getElementById("email").value.trim(); setAuthPanel("forgot"); });
  forgotBack.addEventListener("click",()=>setAuthPanel("login"));
  forgotForm.addEventListener("submit",async(event)=>{ event.preventDefault(); forgotSubmit.disabled=true; forgotStatus.hidden=true; try{ const redirectTo=new URL("./",window.location.href).href; const {error}=await client.auth.resetPasswordForEmail(forgotEmail.value.trim(),{redirectTo}); if(error) throw error; forgotStatus.textContent="קישור לאיפוס סיסמה נשלח. בדוק את תיבת האימייל שלך."; forgotStatus.className="auth-status success"; forgotStatus.hidden=false; }catch(error){ forgotStatus.textContent=`שליחת הקישור נכשלה: ${error.message}`; forgotStatus.className="auth-status error"; forgotStatus.hidden=false; }finally{ forgotSubmit.disabled=false; } });
  recoveryForm.addEventListener("submit",async(event)=>{ event.preventDefault(); recoveryStatus.hidden=true; if(newPassword.value!==confirmPassword.value){ recoveryStatus.textContent="הסיסמאות אינן תואמות."; recoveryStatus.className="auth-status error"; recoveryStatus.hidden=false; return; } recoverySubmit.disabled=true; try{ const {error}=await client.auth.updateUser({password:newPassword.value}); if(error) throw error; recoveryStatus.textContent="הסיסמה עודכנה בהצלחה. ניתן להמשיך למערכת."; recoveryStatus.className="auth-status success"; recoveryStatus.hidden=false; const {data}=await client.auth.getSession(); if(data.session) setTimeout(()=>showReport(data.session),700); }catch(error){ recoveryStatus.textContent=`עדכון הסיסמה נכשל: ${error.message}`; recoveryStatus.className="auth-status error"; recoveryStatus.hidden=false; }finally{ recoverySubmit.disabled=false; } });
  profileForm.addEventListener("submit",async(event)=>{ event.preventDefault(); profileSave.disabled=true; profileStatus.hidden=true; try{ const {data,error}=await client.auth.getSession(); if(error||!data.session) throw error||new Error("אין התחברות פעילה"); const userId=data.session.user.id; const displayName=profileDisplayName.value.trim(); if(displayName.length<2) throw new Error("יש להזין שם באורך שני תווים לפחות"); let avatarPath=null; const file=profileAvatarFile.files[0]; if(file){ if(file.size>3*1024*1024) throw new Error("התמונה גדולה מ־3MB"); if(!["image/png","image/jpeg","image/webp"].includes(file.type)) throw new Error("סוג הקובץ אינו נתמך"); const ext=file.type==="image/png"?"png":file.type==="image/webp"?"webp":"jpg"; avatarPath=`${userId}/avatar.${ext}`; const {error:uploadError}=await client.storage.from("avatars").upload(avatarPath,file,{upsert:true,contentType:file.type,cacheControl:"3600"}); if(uploadError) throw uploadError; }
      const updatePayload={display_name:displayName}; if(avatarPath) updatePayload.avatar_path=avatarPath;
      const {error:updateError}=await client.from("app_users").update(updatePayload).eq("auth_user_id",userId); if(updateError) throw updateError;
      profileStatus.textContent="הפרופיל נשמר בהצלחה."; profileStatus.className="auth-status success"; profileStatus.hidden=false; await updateActiveUser(data.session); setTimeout(closeProfileModal,600);
    }catch(error){ profileStatus.textContent=`שמירת הפרופיל נכשלה: ${error.message}`; profileStatus.className="auth-status error"; profileStatus.hidden=false; }finally{ profileSave.disabled=false; } });
  ...
  document.addEventListener("keydown",(event)=>{ if(event.key==="Escape"){ closeProfileMenu(); closeProfileModal(); } });
  editProfileAction.addEventListener("click",openProfileEditor);
  ...
  client.auth.getSession().then(({data,error})=>{ if(error){ console.error(error); showLogin(); return; } data.session?showReport(data.session):showLogin(); }).catch((error)=>{ console.error(error); showLogin(); });
  client.auth.onAuthStateChange((event,session)=>{ if(event==="PASSWORD_RECOVERY"){ stopRefresh(); appView.hidden=true; loginView.hidden=false; setAuthPanel("recovery"); return; } if(event==="SIGNED_OUT"||!session) showLogin(); else if(event==="SIGNED_IN"||event==="TOKEN_REFRESHED"||event==="USER_UPDATED") updateActiveUser(session); });
})();