(() => {
  ...
  function renderBusinessKpis(report,checkingBalance){
    const settings=report.settings||{}, monthly=report.monthly||{}, sales=Number(report.sales?.income||monthly.sales||0), immediate=Number(monthly.immediate_receipts||monthly.bank_receipts||0), immediateTarget=Number(settings.monthly_revenue_target||57500), dailyTarget=Number(settings.daily_immediate_receipts_target||2500), expenseTarget=Number(settings.monthly_expense_target||20000), checkingTarget=Number(settings.available_cash_target||10000), marginTarget=Number(settings.net_profit_margin_target||.3), expenses=Number(monthly.business_expenses||0), taxes=Number(report.tax?.total_tax_reserve||0);
    ...
    const estimatedProfit=sales-expenses-taxes, margin=sales>0?estimatedProfit/sales:0, checkingGap=checkingBalance-checkingTarget, expenseProgress=expenses>0&&expenseTarget>0?expenses/expenseTarget*100:0, checkingProgress=checkingTarget>0?Math.max(0,checkingBalance/checkingTarget)*100:0, marginProgress=marginTarget>0?margin/marginTarget*100:0;
    ...
    setText("net-margin",expenses>0?`${numberText(margin*100,1)}%`:"חלקי",expenses>0?(margin>=marginTarget?"pos":"warn"):null); setText("net-margin-target",`${numberText(marginTarget*100,0)}%`); setText("net-margin-note",expenses>0?`יעד: ${numberText(marginTarget*100,0)}% לאחר רזרבת מס`:"חישוב חלקי עד שיוחזרו הוצאות חודשיות בפועל"); setProgress("margin-progress-bar",expenses>0?marginProgress:0,margin>=marginTarget?"green":"amber");
  }
  ...
  function renderHealthTrend(report, currentHealth){ const raw=report.health_history||report.weekly_health||[]; let scores=Array.isArray(raw)?raw.map((x)=>Number(x.score??x.health??x)).filter(Number.isFinite):[]; if(!scores.length){ const sales=Number(report.sales?.income||0),target=Number(report.settings?.monthly_sales_target||1); scores=[Math.max(0,currentHealth-12),Math.max(0,currentHealth-8),Math.max(0,currentHealth-4),currentHealth].map((v,i)=>Math.round(Math.min(100,v+(sales/target)*i))); } scores=scores.slice(-4); const bars=document.getElementById("health-weekly-bars"); if(bars) bars.innerHTML=scores.map((s)=>`<span style="height:${Math.max(8,Math.min(100,s))}%" title="${s}/100"></span>`).join(""); const delta=scores.length>1?scores.at(-1)-scores.at(-2):0; setText("health-weekly-trend",delta>0?`+${delta} נק׳`:delta<0?`${delta} נק׳`:"ללא שינוי",delta>0?"pos":delta<0?"neg":"warn"); setText("health-weekly-note",raw.length?"מבוסס על היסטוריית בריאות שבועית":"תחזית זמנית עד שתצטבר היסטוריה שבועית אמיתית"); }

  function renderManagementOverview(report,cashflow){
    const sales=Number(report.sales?.income||0),target=Number(report.settings?.monthly_sales_target||0),checking=Number((report.accounts||[]).find((x)=>x.name==="עו״ש עסק – הבינלאומי")?.balance||0),immediate=Number(report.monthly?.immediate_receipts||0),immediateTarget=Number(report.settings?.monthly_revenue_target||57500),loanBalance=Number(report.loans?.balance||0),expenses=Number(report.monthly?.business_expenses||0),expenseTarget=Number(report.settings?.monthly_expense_target||20000);
    const salesScore=Math.min(target>0?sales/target:0,1), immediateScore=Math.min(immediateTarget>0?immediate/immediateTarget:0,1), coverageScore=Math.min(cashflow.coverage,1), checkingTarget=Number(report.settings?.available_cash_target||10000), checkingScore=Math.max(0,Math.min(checking/checkingTarget,1)), expenseScore=expenses>0?Math.max(0,Math.min(1,expenseTarget/expenses)):.55, debtScore=loanBalance>0?Math.max(.2,1-Math.min(loanBalance/1000000,.8)):.9;
    let health=Math.round((salesScore*.28+immediateScore*.22+coverageScore*.20+checkingScore*.14+expenseScore*.12+debtScore*.04)*100);
    if(cashflow.endingBalance<0) health=Math.min(health,74); if(checking<0) health=Math.min(health,68); if(cashflow.coverage<1) health=Math.min(health,79); if(expenses>expenseTarget&&expenses>0) health=Math.min(health,82); if(loanBalance>0) health=Math.min(health,92); health=Math.max(0,Math.min(health,100));
    setText("business-health-score",`${health}/100`,health>=75?"pos":health>=55?"warn":"neg"); setProgress("business-health-bar",health,health>=75?"green":health>=55?"amber":"red"); setText("business-health-note","משקל גבוה לפעילות שוטפת; חוב ארוך טווח משפיע פחות אך מונע ציון מושלם"); renderHealthTrend(report,health);
    const improvements=[]; if(cashflow.coverage<1) improvements.push(`לסגור פער כיסוי של ${money(Math.max(cashflow.totalExpense-cashflow.totalIncome,0))}`); if(checking<0) improvements.push(`להחזיר את העו״ש מעל 0 ולכיוון יעד ${money(checkingTarget)}`); if(immediateScore<.8) improvements.push("להגדיל תקבולים מיידיים ולהקטין פריסות"); if(expenses===0) improvements.push("להשלים נתוני הוצאות כדי לשפר את אמינות הציון"); if(!improvements.length) improvements.push("לשמור רזרבות ולהמשיך בקצב המכירות הנוכחי"); setText("health-improvement",improvements[0],cashflow.endingBalance<0||checking<0?"warn":"pos"); setText("health-improvement-note",improvements.slice(1).join(" · ")||"הפעילות התפעולית יציבה יחסית");
    if(cashflow.endingBalance<0){setText("primary-insight","פער תזרימי צפוי","neg");setText("primary-insight-note",`היתרה החזויה בסוף התקופה היא ${money(cashflow.endingBalance)}`);}else if(salesScore<1){setText("primary-insight","יעד המכירות עדיין פעיל","warn");setText("primary-insight-note",`נותרו ${money(Math.max(target-sales,0))} ליעד`);}else{setText("primary-insight","יעד המכירות הושג","pos");setText("primary-insight-note",`המכירות מעל היעד ב־${money(sales-target)}`);}
  }
