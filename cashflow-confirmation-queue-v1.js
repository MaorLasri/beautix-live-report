(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .cashflow-confirmation-list{display:grid;gap:10px}.cashflow-confirmation-item{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;padding:14px;border:1px solid #e6e9f0;border-radius:15px;background:#fbfcfe}.cashflow-confirmation-main{min-width:0}.cashflow-confirmation-main strong{display:block;color:#26384d;overflow-wrap:anywhere}.cashflow-confirmation-meta{display:flex;flex-wrap:wrap;gap:6px 10px;margin-top:6px;color:#7a8495;font-size:.82rem}.cashflow-confirmation-type{font-weight:800}.cashflow-confirmation-type.expense{color:#c6434b}.cashflow-confirmation-type.income{color:#268a59}.cashflow-confirmation-actions{display:flex;gap:8px}.cashflow-confirmation-btn{min-height:40px;padding:8px 14px;border:0;border-radius:11px;background:#22945f;color:#fff;font:inherit;font-weight:900;cursor:pointer}.cashflow-confirmation-btn:disabled{opacity:.55;cursor:wait}.cashflow-confirmation-empty{padding:16px;border:1px dashed #d9deea;border-radius:14px;text-align:center;color:#7a8495}.cashflow-confirmation-error{margin:10px 0 0;color:#b33a45;font-weight:700}
  ...
  function render(report) {
    ...
    list.innerHTML = items.map(item => `
      <article class="cashflow-confirmation-item">
        ...
        <div class="cashflow-confirmation-actions"><button type="button" class="cashflow-confirmation-btn" data-confirm-entry="${esc(item.id)}">אישור</button></div>
      </article>`).join('');
