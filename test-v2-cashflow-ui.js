(() => {
  ...
  const state={entries:new Map(),loading:false,openDays:new Set()};
  ...
  async function saveEntry(e){
    ...
    const params={
      p_id:$('cashflow-entry-id').value||null,
      p_date:$('cashflow-entry-date').value,
      p_type:$('cashflow-entry-type').value,
      p_amount:Number($('cashflow-entry-amount').value),
      p_description:$('cashflow-entry-description').value.trim(),
      p_payment_method:$('cashflow-entry-method').value,
      p_status:$('cashflow-entry-status').value
    };
  ...
  function bind(){
    ...
    document.addEventListener('click',e=>{
      ...
      const close=e.target.closest('.dialog-close,.dialog-cancel');if(close){$('cashflow-entry-dialog')?.close();return}
