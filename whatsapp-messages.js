(() => {
  if (window.__beautixWhatsappMessagesLoaded) return;
  window.__beautixWhatsappMessagesLoaded = true;
  ...
  function ensureModal() {
    ...
    style.id = 'whatsapp-message-style';
    style.textContent = `
      ...
      @media(max-width:620px){.whatsapp-message-dialog{padding:22px 16px}.whatsapp-message-actions{flex-direction:column}.whatsapp-message-button{font-size:.88rem}}
    `;
    ...
    document.getElementById('whatsapp-copy-message').addEventListener('click', async event => {
      ...
      const button = event.currentTarget; const old = button.textContent; button.textContent = 'הועתק ✓'; setTimeout(() => button.textContent = old, 1200);
    });
  }
  ...
  function enhance() {
    ...
    document.querySelectorAll('a[href^="tel:"]').forEach(link => {
      ...
      if (!phone || link.parentElement?.querySelector(':scope > .whatsapp-message-button')) return;
      const button = document.createElement('button');
      button.type = 'button'; button.className = 'whatsapp-message-button'; button.textContent = 'WhatsApp';
  ...
  window.addEventListener('beautix:report-loaded', enhance);
})();